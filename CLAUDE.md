...
## Project Plan
- [x] index.css (dark terminal theme, fonts, animations, grid utility)
- [x] astro.config.mjs (Sora + Instrument Sans + JetBrains Mono via Fonts API, site URL, favicon)
- [x] src/assets/favicon.svg (terminal-monogram favicon)
- [x] Layout.astro (cleaned; SITE_NAME/DESCRIPTION, Font preloads, Person-ready)
- [x] index.astro (terminal-window personal card: portrait, bio, skills, "Let's talk!" CTA, socials)
- [x] 404.astro (terminal-styled not-found)
- [x] public/llms.txt + AGENTS.md (updated for Nejc Zupan)
- [x] site-specification.md
